/*
  ...
  /** JVM primitive type descriptors — replaces deleted JavaType.primitiveFor() */
  private val JVM_PRIMITIVE_DESCRIPTORS = setOf('B', 'C', 'D', 'F', 'I', 'J', 'S', 'V', 'Z')
  ...
  class CompletionListAdapter : EditorCompletionAdapter() {
    ...
    private fun methodIdentifier(memberName: String, erasedParameterTypes: List<String>): String {
      ...
      for (type in erasedParameterTypes) {
        if (type.length == 1 && type[0] in JVM_PRIMITIVE_DESCRIPTORS) {
          sb.append(type)
        } else {
